...
import (
	...
	clock "github.com/kzmake/examples/dapr-clock/gen/go/clock/v1"
)
...
func newGatewayServer(ctx context.Context) (*http.Server, error) {
	...
	clockMux := runtime.NewServeMux(
		runtime.WithMetadata(func(_ context.Context, _ *http.Request) metadata.MD {
			return metadata.Pairs("dapr-app-id", "svc-clock")
		}),
	)
	if err := clock.RegisterClockHandlerFromEndpoint(ctx, clockMux, env.Dapr.Address, []grpc.DialOption{grpc.WithInsecure()}); err != nil {
		return nil, xerrors.Errorf("Failed to register handler: %w", err)
	}

	r.Group("/clock").Any("/*any", gin.WrapH(clockMux))
